...
- 每天 4 次（UTC 01:19 / 07:19 / 13:19 / 19:19）定时检测目标用户的全部公开仓库
...
上传 Release 使用 Actions 内置的 `GITHUB_TOKEN`，无需额外配置。
...
```bash
export BACKUP_USERNAME=someuser GITHUB_TOKEN=ghp_xxx
python3 backup.py --dry-run   # 只检测和打包，不上传、不改 state
```
